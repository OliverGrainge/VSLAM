...
class FlannMatcher(ABCFeatureMatcher):
    ...
    def filter_inliers2d(self, camera):
        _, mask = cv2.findEssentialMat(camera.left_kpoints2d,
    ...
    def filter_inliers3d(self, camera):
        ...
        reproj_error = ((np.sqrt(((proj2d_left-camera.left_kpoints2d)**2).sum(axis=1))) + (np.sqrt(((proj2d_right-camera.right_kpoints2d)**2).sum(axis=1))))/2
        mask_x = np.logical_and((camera.kpoints3d[:, 0] > config["Stereo"]["min_x"]), (camera.kpoints3d[:, 0] < config["Stereo"]["max_x"]))
        mask_y = np.logical_and((camera.kpoints3d[:, 1] < config["Stereo"]["max_y"]), (camera.kpoints3d[:, 1] > config["Stereo"]["min_y"]))

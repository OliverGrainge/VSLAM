...
class Map:
    ...
    def __call__(self, camera: StereoCamera, tracking_info):
        ...
        if len(self.cameras) >= self.window: 
            ...
            cost = ba_cost(params, n_cameras, np.zeros(5), kl, obs, da)

            


            """
            pts3d = transform_points3d(query_cam.kpoints3d, query_cam.x)
            x = np.linalg.inv(query_cam.x)
            rvec = cv2.Rodrigues(x[:3, :3])[0]
            tvec = x[:3, 3]
            obs = query_cam.left_kpoints2d
            res_self, _ = reprojection_error(pts3d, rvec, tvec, np.zeros(5), query_cam.kl, obs)
            ...
            for assoc, cam in zip(da, self.cameras[-self.window:-1]):
                ...
                print(np.mean(res), np.mean(res_self))
                """
            """
            matches = np.array(
                [
